...
public final class Board extends Observable {
    ...
    /** CHANGE ONE
     * Bag which starts holding one of each piece, randomly. 
     * This is fed into the Deque until empty, then is refilled with one of each. 
     */
    private List<Piece> myPieceBag;
    ...
    public Board(final int theWidth, final int theHeight,
        ...
        myPieceBag = new ArrayList<>(); //CHANGE TWO
    ...
    private void assignNextPiece() {
        if (myPieces == null || myPieces.isEmpty()) {
            //CHANGE FIVE
            if (myPieceBag.isEmpty()) {
                fillBag(myWidth / 2 - 2, myHeight);
            }
            myNextPiece = randomPiece();
        } else {
    ...
    /** //CHANGE THREE
     * Helper method which fills the piece bag with one of each piece.
     * 
     * @param theX Horizontal coordinate where to place this piece
     * @param theY Vertical coordinate where to place this piece
     */
    private void fillBag(final int theX, final int theY) {
        myPieceBag.add(new IPiece(theX, theY));
        myPieceBag.add(new JPiece(theX, theY));
        myPieceBag.add(new LPiece(theX, theY));
        myPieceBag.add(new OPiece(theX, theY));
        myPieceBag.add(new SPiece(theX, theY));
        myPieceBag.add(new TPiece(theX, theY));
        myPieceBag.add(new ZPiece(theX, theY));
    }

    /** //CHANGE FOUR
     * Chooses a piece randomly from the pieces remaining in the pieceBag.
     * 
     * @return A randomly chosen piece from the bag
     */
    private Piece randomPiece() {
        //Picks randomly from the bag of pieces, removing that piece.
        //The bag will slowly empty, and once it does it will be refilled.
        //Thus, while the order is random, you are guaranteed an even amount of each.
        return myPieceBag.remove(RANDOM.nextInt(myPieceBag.size()));
    }
